...
import pandas as pd
# • N: Number of hiddenlayers in the network
# • A list of size N specifying the number of neurons in each layer
# • lr: Learning rate
# • Activation function (same activation function is used in all layers of the network
# except the last layer)
# • Weight initialization function
# • Number of epochs
# • Batch size

class MultiLayerPerceptron(N,A): 
    ...
    def activation_deriv_fn(self,name,flag=0):
        if(name.equals("sigmoid") & flag == 0):
            return lambda x: 1/(1 + np.exp(-x))

        elif(name.equals("sigmoid") & flag == 1):
            return lambda x: x*(1-x)
        elif(name == ("tanh") & flag == 0):
